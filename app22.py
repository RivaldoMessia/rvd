...
@st.cache_data
def load_data():
    # Simulated crop data as CSV string (based on provided structure)
...
with tab2:
    ...
    factor = st.selectbox("Select Factor", ["Rain Fall (mm)", "Fertilizer", "Temperatue", "Nitrogen (N)", "Phosphorus (P)", "Potassium (K)"],
                         key="factor_select")
    fig = px.scatter(crop_df, x=factor, y="Yield (Q/acre)", trendline="ols", title=f"{selected_crop} Yield vs {factor}",
...
with tab4:
    ...
    if predicted_yield < crop_specific_avg - 0.5:  # Adjusted threshold for small dataset
        recommendations = [
...
with tab5:
    ...
    if st.button("Generate Planting Schedule", key="schedule_button"):
        ...
        if not weather_df.empty:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=weather_df["date"], y=weather_df["temp"], mode="lines+markers", name="Temperature (°C)",
                                    line=dict(color="#FF5722")))
            fig.add_trace(go.Scatter(x=weather_df["date"], y=weather_df["rainfall"], mode="lines+markers", name="Rainfall (mm)",
            ...
            fig.update_layout(
                ...
                yaxis2=dict(title="Rainfall (mm)", titlefont=dict(color="#2196F3"), tickfont=dict(color="#2196F3"), overlaying="y", side="right"),
        ...
        schedule_text = f"Planting Schedule for {crop} in {city}\nBest Period: {date_range}\nDates: {', '.join([d.strftime('%Y-%m-%d') for d in suitable_dates]) if suitable_dates else 'None'}\n\nWeather Forecast:\n{weather_df.to_string(index=False) if not weather_df.empty else 'No data available'}"
        st.download_button(
...
if st.button("Download Full Report", key="full_report_button"):
    ...
    report_text += f"Planting Schedule for {crop} in {city}:\n- Best Period: {date_range}\n- Suitable Dates: {', '.join([d.strftime('%Y-%m-%d') for d in suitable_dates]) if suitable_dates else 'None'}\n\n"
