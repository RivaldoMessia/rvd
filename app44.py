...
with tab5:
    ...
    if st.button("Generate Planting Schedule", key="schedule_button"):
        ...
        if not st.session_state["weather_df"].empty:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=st.session_state["weather_df"]["date"], y=st.session_state["weather_df"]["temp"], mode="lines+markers", name="Temperature (°C)",
                                    line=dict(color="#FF5722")))
            fig.add_trace(go.Scatter(x=st.session_state["weather_df"]["date"], y=st.session_state["weather_df"]["rainfall"], mode="lines+markers", name="Rainfall (mm)",
