...
def get_simulated_weather(city):
    ...
    if df.empty or not all(col in df.columns for col in ["date", "temp", "rainfall"]):
        raise ValueError("Invalid weather data generated")
    return df
...
with tab1:
    ...
    mean_yield = df["Yield (Q/acre)"].mean()
    fig.add_vline(
        x=mean_yield,
        line_dash="dash",
        line_color="black",
        annotation_text=f"Mean Yield: {mean_yield:.2f}",
...
with tab5:
    ...
    if st.button("Generate Planting Schedule", key="schedule_button"):
        try:
            ...
            if st.session_state["weather_df"].empty or not all(col in st.session_state["weather_df"].columns for col in ["date", "temp", "rainfall"]):
                raise ValueError("Weather data is empty or missing required columns")
            suitable_dates = []
        ...
        if not st.session_state["weather_df"].empty and all(col in st.session_state["weather_df"].columns for col in ["date", "temp", "rainfall"]):
            fig = go.Figure()
            ...
            st.warning("No valid weather data available to display.")
